...
global current, platformx, platformlist, coinlist, points, start, platformy, enemy, enemylist, begin, all_sprites_list, newscore,player,userlaser #assigns all values
newscore = 0
begin = 0
enemy = 0
count = 0
...
class Player(Entity):
    ...
    def __init__(self, x, y, width, height):
        ...
        self.ball = pygame.image.load('squid.png')
        self.image = pygame.transform.scale(self.ball, (70, 70))
    ...
    def MoveKeyDown(self, key):
        ...
        if key == pygame.K_LEFT:
            ...
            shoot=PlayerLaser(player.rect.x+35,player.rect.y,5,40)
...
def collision():  # if player jumps onto platform
    global current, platformlist, platformx, platformy, coinlist, enemy, enemylist
    for platform in platformlist:
        end = platform.rect.x + 150
        front = platform.rect.x - 30
        if platform != current:  # if new platform
            if player.rect.x >= front and player.rect.x <= end and player.move == 1:  # checks if x coordinates of player are in platform
...
def restart(): #reassigns all values
    global all_sprites_list, platformy, coinlist, platformlist, laslist, begin, enemy, count, platformx, points, start, enemylist, current,player,newscore,userlaser
    player.rect.y=545
...
jumpsound = pygame.mixer.Sound("swim.wav")
...
while True:
    if begin == 0: #menu
        screen.fill(BLACK)
        title = titlefont.render(str("SWIMMING SQUID"), True, WHITE) #display for menu
        ...
        instruct3 = font.render(str("Each coin 1 point. Killing a monster is 25 points."),True,WHITE)
        instruct4=font.render(str("You lose by falling,touching a monster, or getting hit by a laser."), True, WHITE)
        instruct5 = font.render(str("Use arrows to move left and right. Use spacebar to jump."), True, WHITE)
        instruct6=font.render(str("Use up arrow to shoot."),True,WHITE)
        instruct7 = font.render(str("PRESS ENTER TO RETURN TO HOME SCREEN"), True, WHITE)
        instruct8=smallfont.render(str("Swimming Squid. Version .1"),True,WHITE)
        ...
        instructbox7.center=(250,300)
        instructbox8.center=(250,650)
        ...
        if count % 250 == 0: #shoots lasers periodically
            for monster in enemylist:
                lasersound.play()
                laser = Laser(monster.rect.x + 25, monster.rect.y + 70, 5, 40)
