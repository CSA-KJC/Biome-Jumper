...
pygame.display.set_caption("Biome Jumper")
...
class Land(Entity):  # assigns coins
    def __init__(self, x, y, width, height):
        super(Land, self).__init__(x, y, width, height)
        self.coin = pygame.image.load("sky.png")
        self.image = pygame.transform.scale(self.coin, (250, 350))

class Ocean(Entity):  # assigns coins
    def __init__(self, x, y, width, height):
        super(Ocean, self).__init__(x, y, width, height)
        self.coin = pygame.image.load("ocean.png")
        self.image = pygame.transform.scale(self.coin, (250, 350))
...
def enemycollide():  # if player touches coin
    global enemylist,begin,userlaser,monster,coinlist,points,need
    for x in enemylist:
...
def restart(): #reassigns all values
    global all_sprites_list, platformy, coinlist, platformlist, laslist, begin, enemy, count, platformx, points, start, enemylist, current,player,newscore,userlaser,pic,need
    player.rect.y=545
...
enemydie=pygame.mixer.Sound("enemydie.wav")
music = pygame.mixer.music.load("jumpmusic.wav")
...
while True:
    if begin == 0: #menu
        ...
        screen.fill(BLACK)
        seatheme=titlefont.render(str("Press S for Sea Theme"),True,BLACK)
        landtheme=titlefont.render(str("Press L for Land Theme"),True,BLACK)
        seathemebox=seatheme.get_rect()
        landthemebox=landtheme.get_rect()
        img = pygame.image.load('sky.png')
        img = pygame.transform.scale(img, (500, 350))
        screen.blit(img,(0,0))
        img2=pygame.image.load("ocean.png")
        img2=pygame.transform.scale(img2,(500,350))
        screen.blit(img2,(0,350))
        seathemebox.center=(250,550)
        landthemebox.center=(250,200)
        ...
        instruct5=font.render(str("Enemy lasers increase speed with each enemy killed."),True,WHITE)
        instruct6 = font.render(str("Use arrows to move left and right. Use up arrow to jump."), True, WHITE)
        instruct7=font.render(str("Use space arrow to shoot."),True,WHITE)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:  # program closes
                pygame.quit()
                sys.exit()
            elif event.type == pygame.KEYDOWN and event.key == pygame.K_UP: #user presses spacebar, player jumps
                if player.y_change==0:
                    player.Jump()
